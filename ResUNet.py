...
class EmbedBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, self.input_dim) # x of dimension (N, in)
        return self.layers(x) # output of dim (N, emb_dim)
...
class ConditionalUnet(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        # time step embedding
        temb1 = self.timeembed1(t).view(-1, self.n_feat * 2, 1, 1) # (B, 256, 1, 1)
        temb2 = self.timeembed2(t).view(-1, self.n_feat, 1, 1) # (B, 128, 1, 1)
        ...
        res0 = self.init_conv(x) # (B, n_feat, 28, 28)
        down1 = self.downblock1(res0)
        res1 = self.fusion1(down1, temb2, cemb2)
        
        down2 = self.downblock2(res1)
        res2 = self.fusion2(down2, temb1, cemb1)
        
        up0 = self.upblock0(self.to_vec(res2))
        up1 = self.upblock1(up0, res2)
        fus3 = self.fusion3(up1, temb2, cemb2)
        up2 = self.upblock2(fus3, res1)
        
        fus4 = self.fusion4(up2, temb2, cemb2)
        out = self.outblock(torch.cat((fus4, res0), dim=1))
